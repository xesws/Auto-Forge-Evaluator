"""SQL execution match (Spider / BIRD): row-order-independent multiset."""
...
def verify(output: str, reference: dict) -> dict[str, Any]:
    ...
    if gold_result[0] != "ok":
        note = json.dumps(
            {"unparseable": False, "exception": "GoldError", "gold": list(gold_result)},
            ensure_ascii=False,
        )
        return {"pass": False, "parsed": parsed, "note": note}

    passed = _multiset(pred_result[1]) == _multiset(gold_result[1])
    note = json.dumps({"unparseable": False, "match": passed}, ensure_ascii=False)
    return {"pass": bool(passed), "parsed": parsed, "note": note}
